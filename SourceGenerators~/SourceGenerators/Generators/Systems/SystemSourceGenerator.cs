namespace SourceGenerators.Generators.Systems {
    ...
    public static class SystemSourceGenerator {
        ...
        public static void Generate(SourceProductionContext spc, in SystemToGenerate system, in EquatableArray<SystemUpdateMiddleware> middlewares, in PreprocessorOptionsData options) {
            try {
                var source = Generate(system, middlewares, options);
        ...
        public static string Generate(in SystemToGenerate system, in EquatableArray<SystemUpdateMiddleware> middlewares, in PreprocessorOptionsData options) {
            ...
            using (indent.Scope()) {
                ...
                using (indent.Scope()) {
                    using (MorpehSyntax.ScopedProfile(sb, profilerMarker, "Constructor", indent, isUnityProfiler: options.IsUnityProfiler)) {
                        ...
                        for (int i = 0, length = middlewares.Length; i < length; i++) {
                            var middleware = middlewares[i];
                            sb.AppendIndent(indent).Append(UPDATE_MIDDLEWARE_PREFIX).Append(i).Append(" = new ").Append(middleware.FullTypeName).Append("(\"").Append(system.TypeName).AppendLine("\");");
                        }
                    }
                }
